...
int kanjiToInt(const char* word) {
    if (strcmp(word, "零") == 0) return 0;
    if (strcmp(word, "一") == 0) return 1;
    if (strcmp(word, "二") == 0) return 2;
    if (strcmp(word, "三") == 0) return 3;
    if (strcmp(word, "四") == 0) return 4;
    if (strcmp(word, "五") == 0) return 5;
    if (strcmp(word, "六") == 0) return 6;
    if (strcmp(word, "七") == 0) return 7;
    if (strcmp(word, "八") == 0) return 8;
    if (strcmp(word, "九") == 0) return 9;
    return atoi(word);
}
...
void interpret_line(char* line) {
    if (strlen(line) == 0) return; // 空行は無視

    // 関数定義
    if (strstr(line, "術「")) {
        char name[32], p1[32], p2[32], body[256];
        sscanf(line, "術「%[^」]」(数 %[^、]、数 %[^)]) は %[^。]", name, p1, p2, body);
    ...
    // 変数定義・計算
        ...
        if (sscanf(line, "数 %s は %s と %s の%s と定む。", name, v1, v2, op) == 4) {
            ...
            if (strcmp(op, "和") == 0) r = a + b;
            else if (strcmp(op, "差") == 0) r = a - b;
            else if (strcmp(op, "積") == 0) r = a * b;
            else if (strcmp(op, "商") == 0) r = b != 0 ? a / b : 0;
            setVar(name, r);
            printf("（定義）%s = %d\n", name, r);
        } else {
            char valStr[32];
